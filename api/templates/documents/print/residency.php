...
$residencyYears = $doc['created_at'] ? max(1, floor((time() - strtotime($doc['created_at'])) / 31557600)) : '___';
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <title>Certificate of Residency - <?= htmlspecialchars($fullName) ?></title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .doc-header { text-align: center; margin-bottom: 10mm; border-bottom: 3px double #0d47a1; padding-bottom: 8mm; }
        .doc-header .republic { font-size: 11pt; letter-spacing: 2px; color: #444; }
        .doc-header .province { font-size: 12pt; font-weight: 600; }
        .doc-header .barangay-name { font-size: 18pt; font-weight: 700; color: #0d47a1; text-transform: uppercase; letter-spacing: 3px; margin: 4mm 0; }
        ...
        .doc-title h1 { font-size: 20pt; text-transform: uppercase; letter-spacing: 4px; color: #0d47a1; border-bottom: 2px solid #0d47a1; display: inline-block; padding-bottom: 2mm; }
        ...
        .no-print { font-family: 'Inter', sans-serif; text-align: center; padding: 10px; background: #0d47a1; color: white; }
...
<body>
    ...
    <div class="print-page">
        ...
        <div class="doc-title"><h1>Certificate of Residency</h1></div>
        <div class="doc-body">
            <p class="to-whom">TO WHOM IT MAY CONCERN:</p>
            <p class="indent">
                This is to certify that <span class="highlight-name"><?= htmlspecialchars($fullName) ?></span>,
                <?= $age ?> years old, <?= htmlspecialchars($doc['civil_status'] ?? '________') ?>, Filipino,
                is a bonafide resident of <strong><?= htmlspecialchars($address) ?></strong>,
                <?= htmlspecialchars($doc['purok_sitio'] ?? '') ?>,
                Barangay Sample, City of Cabanatuan, Province of Nueva Ecija,
                for approximately <strong><?= $residencyYears ?> year(s)</strong>.
            </p>
            <p class="indent">
                This certification is being issued upon the request of the above-named person for
                <strong><?= htmlspecialchars($doc['purpose'] ?: 'WHATEVER LEGAL PURPOSE IT MAY SERVE') ?></strong>.
            </p>
            <p class="indent">
